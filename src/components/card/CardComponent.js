...
export default function CardComponent({ data }) {
    ...
    const handleWhatsApp = (phoneNumber, propertyInfo) => {
        // Create a pre-filled message about the property
        const message = `Hi, I'm interested in the ${propertyInfo.description} property in ${propertyInfo.location}. Can you provide more information?`;
        ...
        window.open(`https://wa.me/${919324121271}?text=${encodedMessage}`, '_blank');
    };
                        ...
                        <div className="banner-image">
                            ...
                            {item.banners && item.banners.length > 0 ? (
                                <img
                                    ...
                                    alt={`${item.description} - image ${currentImages[index] + 1}`}
                                />
                            ) : (
                                // Fallback to single banner if no banners array exists
                                <img className='bnr' src={item.banner} alt={item.description} />
                            )}
